...
class HistoryEntry:
    def __init__(self, id: int | None = None, to_name: str = "", to_email: str = "",
                 subject: str = "", body: str = "", status: str = "sent",
                 error: str | None = None, created_at: str | None = None):
        self.id = id
    ...
    def from_row(cls, row) -> "HistoryEntry":
        return cls(id=row["id"], to_name=row["to_name"], to_email=row["to_email"],
                   subject=row["subject"], body=row["body"], status=row["status"],
                   error=row["error"], created_at=row["created_at"])

    def to_dict(self) -> dict:
        return {"id": self.id, "to_name": self.to_name, "to_email": self.to_email,
