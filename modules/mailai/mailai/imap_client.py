...
def _connect(folder: str = "INBOX") -> imaplib.IMAP4_SSL:
    ...
    conn.login(config.imap_username or config.smtp_username, config.imap_password or config.smtp_password)
    conn.select(folder)
    return conn
...
def read_email(uid: int, folder: str = "INBOX") -> Optional[EmailMessage]:
    ...
    try:
        ...
        if folder.lower() == "sent":
            actual_folder = _get_sent_folder(conn)
        
        conn.select(actual_folder)
        uid_bytes = str(uid).encode()
        _, data = conn.fetch(uid_bytes, "(RFC822)")
        if data and data[0]:
            raw_email = data[0][1]
