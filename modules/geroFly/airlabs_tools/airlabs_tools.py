...
def _fetch_airport(iata_code: str) -> dict | None:
    data = _get("/airports", {"iata_code": iata_code})
    results = data.get("response", [])
    return results[0] if results else None


def _fetch_schedules(*, dep_iata: str | None = None, arr_iata: str | None = None, limit: int = 50) -> list[dict]:
...
def _fetch_live_flights(*, dep_iata: str | None = None, arr_iata: str | None = None) -> list[dict]:
    params: dict = {}
    if dep_iata:
        params["dep_iata"] = dep_iata
    if arr_iata:
...
def _fetch_flight(flight_iata: str) -> dict | None:
...
def _fetch_delays(*, dep_iata: str | None = None, arr_iata: str | None = None,
                  delay: int = 30, flight_type: str = "departures") -> list[dict]:
...
def _fetch_airline(iata_code: str) -> dict | None:
